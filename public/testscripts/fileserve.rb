...
class FileServe
  ...
  def command(from, cmd, arg)
    ...
    case cmd
      ...
      when 'ls'
        puts "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA 1111"
        text = ""
        Dir.foreach(@directory) { |file|
        puts "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA xxxx"
          next if file =~ /^\./
          path = "#{@directory}/#{file}"
          text += "#{file} (#{human_readable File.size(path)})\n" if File.file?(path)
        }
        puts "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA 2222"
        say.call(text.strip)
      when 'stat'
        @transfers_lock.synchronize {
