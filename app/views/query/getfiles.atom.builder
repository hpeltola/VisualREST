...
atom_feed do |feed|
 ...
 #   tmp = @host + devfile_url(file).gsub!("%2F", "/")
    feed.entry(file, :id => @host+devfile_url(file), :url=>@host+devfile_url(file)) do |entry|
      ...
      entry.created_at file.devfiles_created_at
      entry.modified_at file.blobs_updated_at
...
#      end
      ...
      entry.essence_uri @host + "/user/" + file.username + "/device/" + file.dev_name + "/essence" + file.path + file.name
      entry.metadatas_uri @host + "/user/" + file.username + "/device/" + file.dev_name + "/metadatas" + file.path + file.name
